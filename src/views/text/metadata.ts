...
const listenEditAowPersonDetailFunc = (t: HTMLElement) => {
  ...
  if (val != savedVal) {
    ...
    const aowPersonId =
      t.closest<HTMLElement>(".aow-person-item")?.dataset.aowpersonid;
  ...
  click: [
    {
      ...
      callback: (t: HTMLElement) => {
        (t.nextSibling as HTMLElement)?.classList.remove("d-none");
      },
    },
    {
      selector: ".add-aow-text-type",
      callback: (t: HTMLElement) => {
        const cont = t.closest<HTMLElement>(".text-type-select-div")!;
        const tt =
          cont.querySelector<HTMLInputElement>(".text-type-select")?.value;
        const status = cont.querySelector<HTMLInputElement>(
          ".text-status-select"
        )?.value;
        const aowN = t.closest<HTMLElement>(".metadata-card")?.dataset.aown;
        const textId = t.closest<HTMLElement>(".metadata-card")?.dataset.textid;
        post(`/aows/add_aow_text_type`, {
          ...
          if (data && data.ok) {
            t.closest<HTMLElement>(".card-content")!
              .querySelector(".text-type-items")
              ?.insertAdjacentHTML(
                "beforeend",
                formatAowTt(data.result, { user: true })
              );
            t.closest<HTMLElement>(".card-content")!
    ...
    },
    {
      ...
      callback: (t: HTMLElement) => {
        ...
        post(`/aows/delete_aow_text_type`, { aow_tt_id: val }).then(
          (data: ApiResult) => {
            if (data && data.ok) {
    ...
    },
    {
      ...
      callback: (t: HTMLElement) => {
        const aowN = t.closest<HTMLElement>(".metadata-card")?.dataset.aown;
        const textId = t.closest<HTMLElement>(".metadata-card")?.dataset.textid;
        post(`/aows/add_aow_person`, { aow_n: aowN, text_id: textId }).then(
          (data: ApiResult) => {
            if (data && data.ok) {
              t.closest<HTMLElement>(".card-content")
                ...
                ?.insertAdjacentHTML(
                  "beforeend",
                  formatAowPerson(data.result, { user: true })
                );
            } else {
    ...
    },
    {
      ...
      callback: (t: HTMLElement) => {
        const aowPersonID = t.closest<HTMLElement>("#delete-aow-person-modal")
          ?.dataset.id;
        delete t.closest<HTMLElement>("#delete-aow-person-modal")?.dataset.id;
        post(`/aows/delete_aow_person`, { aow_person_id: aowPersonID }).then(
          (data: ApiResult) => {
            if (data && data.ok) {
              $<HTMLElement>(
                `.aow-person-item[data-aowpersonid="${aowPersonID}"]`
              )?.remove();
            }
          }
        );
    ...
    },
    {
      ...
      callback: (t: HTMLElement) => {
        const modal = $<HTMLElement>("#delete-aow-person-modal");
        modal!.dataset.id =
          t.closest<HTMLElement>(".aow-person-item")?.dataset.aowpersonid;
    ...
    },
    {
      ...
      callback: (t: HTMLElement) => {
        const modal = $<HTMLElement>("#edit-aow-person-modal");
        modal!.dataset.id =
          t.closest<HTMLElement>(".aow-person-item")?.dataset.aowpersonid;
        modal!.querySelector<HTMLElement>("#existing-person-id")!.innerHTML =
          t.closest<HTMLElement>(".aow-person-item")?.dataset
            .aowpersonpersonid || "";
        modal!.classList.remove("d-none");
      },
    },
    {
      ...
      callback: (t: HTMLElement) => {
        const modal = $<HTMLElement>("#edit-aow-person-modal");
        const personId =
          $<HTMLElement>("#existing-person-id")?.textContent || "";
        const aowPersonId = modal?.dataset.id;
        delete t.closest<HTMLElement>("#edit-aow-person-modal")!.dataset.id;
    ...
    },
    {
      ...
      callback: (t: HTMLElement) => {
        const modal = $<HTMLElement>("#edit-aow-person-modal");
        const tmId = $<HTMLElement>("#new-person-tmid")?.textContent;
        const personName = $<HTMLElement>("#new-person-name")?.textContent;
        const gender = $<HTMLInputElement>(
          'input[name="new-gender"]:checked'
        )?.value;
        const aowPersonId = modal?.dataset.id;
        delete t.closest<HTMLElement>("#edit-aow-person-modal")?.dataset.id;
  ...
  change: [
    {
      ...
      callback: (t: HTMLInputElement) => {
        ...
        const newRole = t
          ...
          ?.closest<HTMLInputElement>("select")?.value;
...
const updateAowPersonCard = (aowPerson: Dict<any>) => {
  let aowPersonEl = $<HTMLElement>(
    `.aow-person-item[data-aowpersonid="${aowPerson.id}"`
  );
  if (aowPersonEl) {
    aowPersonEl.dataset.aowpersonpersonid = aowPerson.person_id;
    aowPersonEl.querySelector<HTMLElement>(
      ".aow-personname"
    )!.innerHTML = `<a href="/person/${aowPerson.person_id}">${aowPerson.name}</a>`;
    aowPersonEl.querySelector<HTMLElement>(".aow-person-tmid")!.innerHTML =
      aowPerson.tm_id;
    let gender = "?";
    ...
    aowPersonEl.querySelector<HTMLElement>(".aow-person-gender")!.innerHTML =
      gender;
    aowPersonEl.querySelector<HTMLElement>(".aow-person-personid")!.innerHTML =
      aowPerson.person_id;
  } else {
    alert("Error");
  }
...
const formatAowPerson = (aowPerson: Dict<any>, user: Dict<any>) => {
  ...
  return `
        <div data-aowpersonpersonid="${
          aowPerson.person_id
        }" data-aowpersonid="${
    aowPerson.id
  }" class="aow-person-item" style="margin-bottom:20px">
            ${dlt}
            <p class="bold centered" style="font-size:11pt;">${role}: <span class="aow-personname"><a href="/person/${
    aowPerson.person_id
  }">${aowPerson.name}</a></span> <span data-uncertain="${
    aowPerson.uncertain || 0
  }" class="${certaintyEdit} badge-small uncertainty-${
    aowPerson.uncertain || 0
  }"></span> ${listenPersonEdit}</p>
            <p class="centered" style="margin-top:-10px;"><span class="badge-small">ID: <span class="aow-person-personid">${
              aowPerson.person_id
            }</span></span> <span class="badge-small">TM ID: <span class="aow-person-tmid">${
    aowPerson.tm_id || ""
  }</span></span> <span class="badge-small">Gender: <span class="aow-person-gender">${gender}</span></span></p>
                ...
                <tr>
                    <td data-value="${
                      aowPerson.domicile || ""
                    }" data-item="domicile" ${ed}>${
    aowPerson.domicile || ""
  }</td>
                    <td data-value="${
                      aowPerson.ethnic || ""
                    }" data-item="ethnic" ${ed}>${aowPerson.ethnic || ""}</td>
                    <td data-value="${
                      aowPerson.honorific || ""
                    }" data-item="honorific" ${ed}>${
    aowPerson.honorific || ""
  }</td>
                </tr>
                </table>
                ...
                <tr>
                    <td data-value="${
                      aowPerson.handwriting || ""
                    }" data-item="handwriting" ${ed}>${
    aowPerson.handwriting || ""
  }</td>
                    <td data-value="${
                      aowPerson.education || ""
                    }" data-item="education" ${ed}>${
    aowPerson.education || ""
  }</td>
                    <td data-value="${
                      aowPerson.occupation || ""
                    }" data-item="occupation" ${ed}>${
    aowPerson.occupation || ""
  }</td>
                </tr>
                </table>
...
const formatAowTt = (aow_tt: Dict<any>, user: Dict<any>) => {
  ...
  const status: number = aow_tt.status;
  if (ttName) {
    return `    
...
const formatMetadata = (data: any, user: Dict<any>, docId: number) => {
  let html = "";
  data.forEach((aow: any) => {
    html += `
        <div class="card metadata-card" data-textid="${docId}" data-aown="${
      aow.aow_n
    }" >
            <div class="card-header"><div class="card-title"><span style="font-size:2.5em; position:relative;top:4px;margin-right:4px"><span class="sym">✍</span></span> ${
              aow.hand
            }</div></div>
...
export default async (doc: any, user: Dict<any>) => {
  return post(`/aows/`, { text_id: doc.id }).then((data: ApiResult) => {
