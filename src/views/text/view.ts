...
export default async (doc: any) => {
  return get(`/text/${doc.id}/tokens`).then((data: ApiResult) => {
    if (data && data.ok && data.result.length) {
      return `<div style="text-align:center;"><div style="padding-left:60px; display: inline-block; text-align: left;">${formatTokens(
        data.result
      )}</div></div>`;
    } else {
