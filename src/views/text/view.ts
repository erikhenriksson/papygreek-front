  ...
  change: [
    {
      ...
      callback: (t: HTMLInputElement) => {
        const container = $<HTMLElement>("#text-view");
        const result =
          t.value == "viewhtml"
            ? `
...
const formatHtml = function (tokens: Dict<any>) {
  ...
  tokens.forEach((t: Dict<any>) => {
    ...
    let lineChanged =
      t.line && t.line != "None" && t.line.split("-")[0] != line;
...
const formatTokens = function (tokens: Dict<any>) {
  let sentence = tokens[0].sentence_n;
  let line = tokens[0].line;
  let tokenHtml = `<span class="sentence"><span class="line-margin">${line}:</span>`;
...
export default async (doc: any) => {
  return get(`/text/${doc.id}/tokens_html`).then((data: ApiResult) => {
