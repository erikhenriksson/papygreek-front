...
import { ApiResult, WorkflowHeader, Listeners } from "../../types.js";
  ...
  click: [
    {
      ...
      callback: (t: HTMLElement) => {
        const dataContainer = t.closest<HTMLElement>(".status-buttons");
...
const getHeader = (header: WorkflowHeader, status: string) => {
  let approved = "";
  let annotated = "";
...
export default async (doc: any, user: any) => {
  return get(`/text/${doc.id}/workflow`).then((data: ApiResult) => {
    if (data && data.ok) {
      return `
        <div class="grid">
            <div class="g-6">
                <div id="orig-workflow" class="card parent-card card-full">
                    <div class="card-header">
                        ...
                        ${getButtons(
                          ...
                          user,
                          "orig",
                          data.result["orig_status"]
                        )}
                    </div>
                    <div class="card-content">
                        ${getHeader(
                          data.result["orig_header"],
                          data.result["orig_status"]
                        )}
                    </div>
                </div>
                ...
                <div id="reg-workflow" class="card parent-card card-full">
                    <div class="card-header">
                        ...
                        ${getButtons(
                          ...
                          user,
                          "reg",
                          data.result["reg_status"]
                        )}
                    </div>
                    <div class="card-content">
                        ${getHeader(
                          data.result["reg_header"],
                          data.result["reg_status"]
                        )}
                    </div>
                </div>
