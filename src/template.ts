...
interface ExtendedChildNode extends ChildNode {
  nodes: Dict<any>;
}

interface ExtendedDocumentFragment extends DocumentFragment {
  nodes: Dict<any>;
}

export default (html: string | HTMLElement, scope: Dict<any>) => {
  ...
  const toScope = (node: Element, scope: Dict<any>) => {
  ...
  if (node.childNodes.length == 1) {
    ...
    if (!scope) {
      (node.childNodes[0] as ExtendedChildNode).nodes = _scope;
    }
  ...
  (fragment as ExtendedDocumentFragment).nodes = _scope;
  return fragment;
};
