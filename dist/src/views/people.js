        ...
        if (data && data.ok) {
            const result = data.result
                ...
                const roles = el.roles
                    ...
                    : "";
                return `
            <div style="width:100%; break-inside: avoid-column; display:table;" class="tight filterable" data-filterval="${el.name || "[Anonymous]" + el.id}"><h3 style="margin-bottom:4px;margin-top:14px; ${expanded ? "font-weight:bold;" : ""}"><a href="/person/${el.id}"><span>${el.name || "[Anonymous]"}</span></a> <span class="badge-small badge-grey">${el.id}</span>${expanded ? "" : roles}</h3>
          ...
          ${expanded
                    ? (el.texts || "")
                        ...
                        if (t) {
                            ...
                            return `<span><a href="/text/${tParts[2]}">${tParts[1]}</a> <span class="badge-small">${tParts[0]}</span> <span data-uncertain="${tParts[3] || 0}" class="badge-small uncertainty-${tParts[3] || 0}"></span></span><br>`;
                        }
                        else {
