    ...
    click: [
        {
            ...
            callback: (t) => {
                ...
                let closestSentence = t.closest("div.sentence");
                    ...
                    if (data && data.ok) {
                        ...
                        if (!annotationStatus) {
                            ...
                            modalContainer.innerHTML = matches
                                .map((r, i) => {
                                let sHtml = `<div class="sentence" data-import="${r.will_import}" data-sentenceid="${i}"><h4 class="bold" style="margin-bottom:2px;">Sentence ${i + 1}</h4>`;
                                if (r.score == 1) {
                                    if (r.old_sentence == i) {
                                        sHtml += `<p class="info">Ok</p>`;
                                    }
                                    else {
                                        sHtml += `<p class="info">Matches Sentence ${r.old_sentence + 1}</p>`;
                                    }
                                }
                                else {
                                    if (r.score < 0.9) {
                                        ...
                                        sHtml += `<p class="red" style="margin-bottom:4px;"><strong>${score}%</strong> match with <strong>Sentence ${r.old_sentence + 1}</strong></p>`;
...
const getDiffHtml = (edited, original) => {
    // @ts-ignore
...
export default async (doc, user) => {
    return `${!isEmpty(user)
        ...
        : ""}
        <div class="card">
            <div class="card-header"><div class="card-title">Source XML (PapyGreek) ${!isEmpty(user)
        ? `<span id="save-xml" class="button button-small d-none"><span>Save</span>`
        : ""}</div></div>
            <div class="card-content xml"><xmp id="editable-xml" ${!isEmpty(user) ? `contenteditable="true"` : ""}>${doc.xml_papygreek}</xmp></div>
            <xmp id="original" class="d-none" contenteditable="true">${doc.xml_papygreek}</xmp>
        </div>
        <div class="card">
            <div class="card-header"><div class="card-title">Source XML (Original)</div></div>
           <div id="diffcontainer">${getDiffHtml(doc.xml_papygreek, doc.xml_original)}</div>
        </div>
